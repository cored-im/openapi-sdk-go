...
import (
	...
	"github.com/cored-im/openapi-sdk-go/internal/model"
...
func (c *defaultApiClient) Request(ctx context.Context, req *ApiRequest) (*ApiResponse, error) {
	...
	if c.config.EnableEncryption && req.WithAppAccessToken {
		...
		httpReq.Header.Add("User-Agent", UserAgent)
		httpReq.Header.Add("X-Cored-Timestamp", strconv.FormatInt(c.config.TimeManager.GetServerTimestamp(), 10))
		httpReq.Header.Add("X-Cored-Nonce", c.cryptoManager.getNonce())
